...
export default class MilestoneLit extends LitElement {
  ...
  render() {
    const issuesList = this._issues.fetching 
      .then(() => html`<issues-list .issues=${this._issues.value}></issues-list>`)
    
    return html`
    <div id="title">
      <h2>${this.title}</h2>
      <button @click=${this._clickHandler}>
        Запросить задачи</button>
    </div>
